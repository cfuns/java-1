...
public class NoteGuiServletUnitTest {
	...
	public void testService() throws Exception {
		...
		final NoteGuiServlet noteServlet = new NoteGuiServlet(logger, calendarUtil, timeZoneUtil, parseUtil, authenticationService, navigationWidget, httpContextProvider,
			urlUtil, authorizationService, cacheService);

		noteServlet.service(request, response);
		final String content = sw.getBuffer().toString();
		assertNotNull(content);
		assertTrue(content.indexOf("<h1>Note</h1>") != -1);
	}
}
