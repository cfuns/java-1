...
public class RawDecoder extends Decoder {
	...
	public void decode(final Reader reader, final Renderer renderer, final int x, final int y, final int width, final int height) throws TransportException {
		final int length = width * height * renderer.getBytesPerPixel();
		final byte[] bytes = ByteBuffer.getInstance().getBuffer(length);
