...
public class DnsGuiServletUnitTest {
	...
	public void testService() throws Exception {
		...
		final DnsGuiServlet dnsServlet = new DnsGuiServlet(logger, calendarUtil, timeZoneUtil, parseUtil, authenticationService, navigationWidget, httpContextProvider,
			urlUtil, authorizationService, cacheService);

		dnsServlet.service(request, response);
		final String content = sw.getBuffer().toString();
		assertNotNull(content);
		assertTrue(content.contains("<h1>Dns</h1>"));
	}
}
