...
public class DistributedIndexGuiServletUnitTest {
	...
	public void testService() throws Exception {
		...
		final DistributedIndexGuiServlet distributed_indexServlet = new DistributedIndexGuiServlet(logger, calendarUtil, timeZoneUtil, parseUtil, authenticationService,
			navigationWidget, httpContextProvider, urlUtil, authorizationService, distributedIndexGuiLinkFactory, cacheService);

		distributed_indexServlet.service(request, response);
		final String content = sw.getBuffer().toString();
		assertNotNull(content);
		assertTrue(content.indexOf("<h1>DistributedIndex</h1>") != -1);
	}
}
