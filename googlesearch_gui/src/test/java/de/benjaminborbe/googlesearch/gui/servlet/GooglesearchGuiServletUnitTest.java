...
public class GooglesearchGuiServletUnitTest {
	...
	public void testService() throws Exception {
		...
		final GooglesearchGuiServlet googlesearchServlet = new GooglesearchGuiServlet(logger, calendarUtil, timeZoneUtil, parseUtil, authenticationService, navigationWidget,
				httpContextProvider, redirectUtil, urlUtil);

		googlesearchServlet.service(request, response);
		final String content = sw.getBuffer().toString();
		assertNotNull(content);
		assertTrue(content.indexOf("<h1>Googlesearch</h1>") != -1);
	}
}
