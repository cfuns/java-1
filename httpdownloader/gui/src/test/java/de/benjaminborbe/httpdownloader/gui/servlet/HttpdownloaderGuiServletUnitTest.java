...
public class HttpdownloaderGuiServletUnitTest {
	...
	public void testService() throws Exception {
		...
		final HttpdownloaderGuiServlet httpdownloaderServlet = new HttpdownloaderGuiServlet(logger, calendarUtil, timeZoneUtil, parseUtil, authenticationService, navigationWidget, httpContextProvider,
			urlUtil, authorizationService, cacheService);

		httpdownloaderServlet.service(request, response);
		final String content = sw.getBuffer().toString();
		assertNotNull(content);
		assertTrue(content.contains("<h1>Httpdownloader</h1>"));
	}
}
