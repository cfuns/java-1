...
public class ProxyGuiServletUnitTest {
	...
	public void testService() throws Exception {
		...
		final ProxyGuiServlet proxyServlet = new ProxyGuiServlet(logger, calendarUtil, timeZoneUtil, parseUtil, authenticationService, navigationWidget, httpContextProvider,
			urlUtil, authorizationService, cacheService);

		proxyServlet.service(request, response);
		final String content = sw.getBuffer().toString();
		assertNotNull(content);
		assertTrue(content.indexOf("<h1>Proxy</h1>") != -1);
	}
}
