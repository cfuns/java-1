...
public class MicroblogGuiServletUnitTest {
	...
	public void testService() throws Exception {
		...
		final MicroblogGuiServlet microblogServlet = new MicroblogGuiServlet(logger, calendarUtil, timeZoneUtil, parseUtil, authenticationService, navigationWidget,
			httpContextProvider, microblogRevisionStorage, urlUtil, authorizationService, microblogGuiLinkFactory, cacheService);

		microblogServlet.service(request, response);
		final String content = sw.getBuffer().toString();
		assertNotNull(content);
		assertTrue(content.contains("<h1>Microblog</h1>"));
	}
}
