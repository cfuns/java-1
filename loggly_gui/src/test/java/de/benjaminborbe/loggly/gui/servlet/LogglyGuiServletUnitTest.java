...
public class LogglyGuiServletUnitTest {
	...
	public void testService() throws Exception {
		...
		EasyMock.expect(request.getParameter(LogglyGuiConstants.PARAMETER_LOG_MSG)).andReturn(null).anyTimes();
		...
		final LogglyGuiServlet logglyServlet = new LogglyGuiServlet(logger, calendarUtil, timeZoneUtil, parseUtil, authenticationService, navigationWidget, httpContextProvider,
			urlUtil, authorizationService, logglyService, cacheService);

		logglyServlet.service(request, response);
		final String content = sw.getBuffer().toString();
		assertNotNull(content);
		assertTrue(content.indexOf("<h1>Loggly</h1>") != -1);
	}
}
