...
public class FilestorageGuiServletUnitTest {
...
EasyMock.replay(authorizationService);
		...
		final FilestorageGuiServlet filestorageServlet = new FilestorageGuiServlet(logger, calendarUtil, timeZoneUtil, parseUtil, authenticationService, navigationWidget,
				httpContextProvider, redirectUtil, urlUtil, authorizationService);

		filestorageServlet.service(request, response);
		final String content = sw.getBuffer().toString();
		assertNotNull(content);
		assertTrue(content.indexOf("<h1>Filestorage</h1>") != -1);
	}
}
