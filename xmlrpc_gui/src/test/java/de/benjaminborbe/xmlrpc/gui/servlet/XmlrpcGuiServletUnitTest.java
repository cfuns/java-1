...
public class XmlrpcGuiServletUnitTest {
	...
	public void testService() throws Exception {
		...
		final XmlrpcGuiServlet xmlrpcServlet = new XmlrpcGuiServlet(logger, calendarUtil, timeZoneUtil, parseUtil, authenticationService, navigationWidget, httpContextProvider,
				redirectUtil, urlUtil, authorizationService, cacheService);

		xmlrpcServlet.service(request, response);
		final String content = sw.getBuffer().toString();
		assertNotNull(content);
		assertTrue(content.indexOf("<h1>Xmlrpc</h1>") != -1);
	}
}
