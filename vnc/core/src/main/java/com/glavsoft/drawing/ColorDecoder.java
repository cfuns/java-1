...
public class ColorDecoder {
	...
	public ColorDecoder(final PixelFormat pf) {
		...
		isTightSpecific = 4 == bytesPerPixel && 3 == bytesPerPixelSignificant && 255 == redMax && 255 == greenMax && 255 == blueMax;
	}

	protected int readColor(final Reader reader) throws TransportException {
		return getColor(reader.readBytes(buff, 0, bytesPerPixel), 0);
	}

	protected int readCompactColor(final Reader reader) throws TransportException {
		return getCompactColor(reader.readBytes(buff, 0, bytesPerPixelSignificant), 0);
	}

	protected int readTightColor(final Reader reader) throws TransportException {
		return getTightColor(reader.readBytes(buff, 0, bytesPerPixelSignificant), 0);
	}

	protected int convertColor(final int rawColor) {
		return 255 * (rawColor >> redShift & redMax) / redMax << 16 | 255 * (rawColor >> greenShift & greenMax) / greenMax << 8 | 255 * (rawColor >> blueShift & blueMax) / blueMax;
	}

	public void fillRawComponents(final byte[] comp, final byte[] bytes, final int offset) {
		final int rawColor = getRawTightColor(bytes, offset);
		comp[0] = (byte) (rawColor >> redShift & redMax);
		comp[1] = (byte) (rawColor >> greenShift & greenMax);
		comp[2] = (byte) (rawColor >> blueShift & blueMax);
	}

	protected int getTightColor(final byte[] bytes, final int offset) {
		return convertColor(getRawTightColor(bytes, offset));
	}

	private int getRawTightColor(final byte[] bytes, int offset) {
		if (isTightSpecific)
	...
	protected int getColor(final byte[] bytes, final int offset) {
		return convertColor(getRawColor(bytes, offset));
	}

	private int getRawColor(final byte[] bytes, int offset) {
		int shift = startShift;
		final int item = addShiftItem;
	...
	protected int getCompactColor(final byte[] bytes, int offset) {
