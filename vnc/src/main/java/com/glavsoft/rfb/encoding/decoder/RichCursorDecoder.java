...
public class RichCursorDecoder extends Decoder {
	...
	public void decode(final Reader reader, final Renderer renderer, final FramebufferUpdateRectangle rect) throws TransportException {
		final int bytesPerPixel = renderer.getBytesPerPixel();
		final int length = rect.width * rect.height * bytesPerPixel;
		if (0 == length)
			return;
		final byte[] buffer = ByteBuffer.getInstance().getBuffer(length);
		...
		final int scanLine = (int) Math.floor((rect.width + 7) / 8);
		final byte[] bitmask = new byte[scanLine * rect.height];
		...
		final int[] cursorPixels = new int[rect.width * rect.height];
		for (int y = 0; y < rect.height; ++y) {
			for (int x = 0; x < rect.width; ++x) {
				final int offset = y * rect.width + x;
				cursorPixels[offset] = isBitSet(bitmask[y * scanLine + x / 8], x % 8) ? 0xFF000000 | renderer.getPixelColor(buffer, offset * bytesPerPixel) : 0; // transparent
			}
		}
		renderer.createCursor(cursorPixels, rect);
	}

	private boolean isBitSet(final byte aByte, final int index) {
