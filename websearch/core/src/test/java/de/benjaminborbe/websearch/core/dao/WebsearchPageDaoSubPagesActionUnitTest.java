...
public class WebsearchPageDaoSubPagesActionUnitTest {
	...
	public void testFindSubPages() throws Exception {
		...
		{
			final Set<WebsearchPageBean> subPages = new HashSet<>(pageDaoSubPagesAction.findSubPages(url1,
				new EntityIteratorImpl<>(allPages.iterator())));
		...
		{
			final Set<WebsearchPageBean> subPages = new HashSet<>(pageDaoSubPagesAction.findSubPages(url2,
				new EntityIteratorImpl<>(allPages.iterator())));
		...
		{
			final Set<WebsearchPageBean> subPages = new HashSet<>(pageDaoSubPagesAction.findSubPages(url3,
				new EntityIteratorImpl<>(allPages.iterator())));
