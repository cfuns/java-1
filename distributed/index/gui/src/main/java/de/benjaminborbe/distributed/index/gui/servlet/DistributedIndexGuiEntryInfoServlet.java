...
@Singleton
public class DistributedIndexGuiEntryInfoServlet extends WebsiteHtmlServlet {

	private static final long serialVersionUID = 1328676176772634649L;

	private static final String TITLE = "DistributedIndex - Entry-Info";
	...
	@Inject
	public DistributedIndexGuiEntryInfoServlet(
		final Logger logger,
	...
	protected Widget createContentWidget(final HttpServletRequest request, final HttpServletResponse response, final HttpContext context) throws IOException,
		...
		try {
			...
			final String url = request.getParameter(DistributedIndexGuiConstants.PARAMETER_URL);
			if (index != null && url != null) {
				...
				final Map<String, Integer> data = distributedIndexService.getWordRatingForEntry(index, url);

				final TableWidget table = new TableWidget();
				table.addClass("sortable");
				{
					final TableRowWidget row = new TableRowWidget();
					row.addCell(new TableCellHeadWidget("Word"));
				...
				for (final Entry<String, Integer> e : data.entrySet()) {
					...
					{
						final TableCellWidget cell = new TableCellWidget(distributedIndexGuiLinkFactory.wordInfo(request, word, index, word));
						cell.addAttribute("sorttable_customkey", word);
						row.addCell(cell);
					}
					{
						final TableCellWidget cell = new TableCellWidget(distributedIndexGuiLinkFactory.wordInfo(request, rating, index, word));
			...
			formWidget.addFormInputWidget(new FormInputTextWidget(DistributedIndexGuiConstants.PARAMETER_URL).addLabel("Url"));
