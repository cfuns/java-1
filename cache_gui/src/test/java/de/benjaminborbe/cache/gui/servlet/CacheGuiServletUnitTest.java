...
public class CacheGuiServletUnitTest {
	...
	public void testService() throws Exception {
		...
		final CacheGuiServlet cacheServlet = new CacheGuiServlet(logger, calendarUtil, timeZoneUtil, parseUtil, authenticationService, navigationWidget, httpContextProvider,
				redirectUtil, urlUtil, authorizationService, cacheService);

		cacheServlet.service(request, response);
		final String content = sw.getBuffer().toString();
		assertNotNull(content);
		assertTrue(content.indexOf("<h1>Cache</h1>") != -1);
	}
}
