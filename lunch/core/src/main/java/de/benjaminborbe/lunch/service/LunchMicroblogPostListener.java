...
public class LunchMicroblogPostListener implements MicroblogPostListener {
	...
	private boolean isLunch(final MicroblogPost microblogPost) {
		...
		if (calendar == null) {
			logger.debug("can't check isLunch, post.date is null");
			return false;
		}
		...
		if (logger.isDebugEnabled()) {
			logger.debug("isLunch - content: " + content + " date: " + calendarUtil.toDateTimeZoneString(calendar) + " now: "
				+ calendarUtil.toDateTimeZoneString(now) + " diff: " + diff);
		}
		if (diff > MAX_AGE) {
