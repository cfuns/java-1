...
public class LunchWikiConnectorImpl implements LunchWikiConnector {
	...
	private boolean isLunchDateTodayOrFuture(final RemotePageSummary remotePageSummary, final Calendar date) throws ParseException {
		...
		logger.trace("compare " + calendarUtil.toDateTimeString(date) + " <=> " + calendarUtil.toDateTimeString(pageDate));
	...
	protected Lunch createLunch(
		...
		lunch.setName(lunchParseUtil.extractLunchNameFromTitleOrContent(remotePageSummary.getTitle(), htmlContent));
	...
	protected boolean isLunchTitle(final String title) {
